        ...
        <>
            <div className="flex border border-black w-[60vw] h-[50vh] text-white rounded-s-md">
                <div className="bg-black w-[50%] pl-5 pr-5 h-[100%] flex flex-col items-center justify-center">
                    {login ? null : <><Input name="username" placeholder="username" className="rounded border border-light  bg-lightt" value={signData.username} onChange={(e) => handleChange(e)} /><br /></>}
                    <Input name="email" placeholder="email" className="rounded border border-light bg-lightt" value={login ? loginData.email : signData.email} onChange={(e) => handleChange(e)} /><br />
                    <Input name="password" placeholder="password" className="rounded border border-light  bg-lightt" value={login ? loginData.password : signData.password} onChange={(e) => handleChange(e)} /><br /><br />
                ...
                <div className="border border-black w-[30vw] flex flex-col items-center justify-center bg-gradient-to-br from-[#ffd700] via-[#f0c14b] to-[#b8860b] text-black rounded-s-2xl rounded-e-md text-center">
                    ...
                    <Button variant="bbb"><Link to={`/${login ? "signup" : "login"}`} className="text-white bg-black">{!login ? "LOGIN" : "SIGN UP"}</Link></Button>
                </div>
            </div>
        </>
    )
