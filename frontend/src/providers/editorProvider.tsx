...
// Updated function to handle styles as a partial object
const findElemAndUpdate = (container: EditorContainerType, index: string, style: React.CSSProperties): boolean => {
    if (container.id === index) {
        // Merge the existing styles with the new styles
        container.styles = { ...container.styles, ...style };
        return true;
    }
    if (Array.isArray(container.contents)) {
        for (const item of container.contents) {
            const found: boolean = findElemAndUpdate(item as EditorContainerType, index, style);
            if (found) return found;
        }
    }
    return false;
...
const EditorProvider = ({ children }: React.PropsWithChildren) => {
    ...
    const { data, isLoading, error } = useQuery({
        queryKey: ["getWebsite", id], // Add id to queryKey for caching
        queryFn: async () => {
